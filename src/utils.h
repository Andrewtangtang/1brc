...
INLINE int16_t min16(int16_t a, int16_t b) {
  ...
  return a < b ? a
               : b; // #2 compiler optimizes without branches, somewhat better
}

INLINE int16_t max16(int16_t a, int16_t b) {
  // return a - ((a - b) & ((a - b) >> 15)); // #1 branchless but slower
  return a < b ? b
               : a; // #2 compiler optimizes without branches, somewhat better
}

INLINE int64_t min64(int64_t a, int64_t b) {
  // return b + ((a - b) & ((a - b) >> 63)); // #1 branchless, similar speed
  // (used less frequently than min16)
...
INLINE size_t move_file_split_after_newline(size_t split_end_pos, size_t fsize,
                                            const int fd,
  ...
  const int ret =
      lseek(fd, split_end_pos - 1,
            SEEK_SET); // seek to byte before split (we need it to be newline)
  ...
  for (; buffer[newline_offset] != '\n' && newline_offset < max_record_size;
       newline_offset++)
    ;
...
INLINE size_t move_buff_split_after_newline(size_t split_end_pos, size_t fsize,
                                            char *buffer) {
  if (split_end_pos >= fsize)
    return fsize;
