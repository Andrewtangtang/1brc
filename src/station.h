...
#ifdef SAFE
  int64_t sum; // PERF: Slower, likely due to cache misses; This is totaly safe,
               // should work even with data from Antarctida or Venus
#else
  int32_t sum; // PERF: Faster; For real-world temperatures and using threads
               // should be enough
#endif
...
INLINE station_data_t *get_station(stations_t *stations, char *name,
                                   size_t name_len) {
...
static inline void merge_station(stations_t *merged_stations,
                                 station_data_t *other_station,
                                 stations_t *other_stations, uint16_t index2) {
...
loop:
  ...
  if (LIKELY(index != 0)) {
...
#ifdef SAFE
    if (UNLIKELY(strcmp(merged_stations->names[index],
                        other_stations->names[index2]) != 0)) {
#else
    if (UNLIKELY(s->hash != hash)) {
#endif
      mhash++;
    ...
    s = &merged_stations->data[index];
    s->hash = hash;
    memcpy(merged_stations->names[index], other_stations->names[index2],
           MAX_STATION_NAME);
  }
...
static inline void merge_stations(stations_t *merged_stations,
                                  stations_t *other_stations) {
  // 0 station is not used
  for (uint16_t i = 1; i <= other_stations->count; i++)
    merge_station(merged_stations, &(other_stations->data[i]), other_stations,
                  i);
}
...
static inline void print_sorted_stations_as_json(stations_t *stations,
                                                 bool debg) {
  ...
  qsort(entries + 1, stations->count, sizeof(station_sort_entry_t),
        compare_station_name);
