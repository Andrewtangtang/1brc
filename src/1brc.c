...
int main(int argc, char *argv[]) {
    ...
    // Open a separate file descriptor for boundary scanning that doesn't use O_DIRECT
    ...
    for (int i = 0; i < cfg.cpus; i++) {
        pos_end =  cfg.mmap ? move_buff_split_after_newline(pos + seg_size, fsize, mmap_buf)
                            : move_file_split_after_newline(pos + seg_size, fsize, scan_fd, MAX_RECORD_BYTES);
        ...
        if (cfg.iopoll && !cfg.mmap && i < cfg.cpus - 1) {
            ...
            size_t aligned_pos_end = (pos_end + O_DIRECT_ALIGNMENT - 1) & ~(O_DIRECT_ALIGNMENT - 1);
            
            // Move to next newline after alignment, but only if there's enough space
            if (aligned_pos_end < fsize - MAX_RECORD_BYTES) {
                pos_end = move_file_split_after_newline(aligned_pos_end, fsize, scan_fd, MAX_RECORD_BYTES);
            }
        }
