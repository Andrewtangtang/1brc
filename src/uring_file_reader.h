...
typedef struct {
  ...
  uint16_t lengths[RING_ENTRIES]; // bytes read for every buffer (all should be
                                  // READ_BLOCK_LEN, except the very last)
...
static inline ring_file_reader_t rfr_create(int fd, size_t fstart,
  ...
  int ret = io_uring_queue_init_params(RING_ENTRIES + RING_EXTRA_ENTRIES,
                                       &self.ring, &params);
  // int ret = io_uring_queue_init(RING_ENTRIES + RING_EXTRA_ENTRIES,
  // &self.ring, 0);
...
static inline bool rfr_request_next_block(ring_file_reader_t *reader) {
  ...
  // io_uring_sqe_set_flags(sqe, IOSQE_IO_LINK); // PERF: with it - 20-40%
  // slower (sometimes much slower) with a cahced file
...
static inline uint8_t rfr_request_next_blocks(ring_file_reader_t *reader) {
  for (reader->blocks_in_queue = 0;
       reader->blocks_in_queue < RING_ENTRIES && rfr_request_next_block(reader);
  ...
  // Imitate ring buffer
  memcpy(reader->buf_start,
         reader->buf_start + reader->buf_len - RING_BUFFER_WRAPUP_LEN,
         RING_BUFFER_WRAPUP_LEN);
